...
def sunae(year, day, hour, lat, long):
    ...
    delta = year - 1949.0
    leap = int(delta / 4.0)
    jd = 32916.5 + delta * 365. + leap + day + hour / 24  # 1st no. is mid. 0 jan 1949 minus 2.4e6: leap = leap days since 1949

    # calculate ecliptic coordinates
    time = jd - 51545.0  # 51545.0 + 2.4e6 = noon 1 jan 2000


    mnlong = 280.460 + 0.9856474 * time
    mnlong = mnlong % 360.0  # force mean longitude between 0 and 360 degs
    ...
    # if az = 90 degs. el critical = asin(sin(dec)/sin(lat))
    ...
    # calculate refraction correction for US stan. atmosphere c need to have el in degs before calculating correction
    el = el / rad
    if el > -0.56:
    ...
    az = az / rad
    lat = lat / rad
    # mnlong in degs, gmst in hours, jd in days if 2.4e6 added; mnanom, eclong, oblqec, ra, dec, Imst. and ha in radians
    return az, el
